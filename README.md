...
- **感悟模式**勾选框：无文字纯情绪意象图，散文/随笔适用（等同命令行 `--essay`）
- **带 IP 角色**开关：仅当 `private/ip/` 放了参考图时出现，勾选后选参考图、自动锁定 OpenAI 后端（IP 参考图融合仅 OpenAI 支持，见「IP 角色（进阶）」）
- 风格下拉**自动扫描** `presets/`（及本地 `private/`）目录——新增预设文件后刷新页面即出现，无需改代码
...
| 文件 | 作用 |
|---|---|
| `art_director.md` | 设计哲学 + 工作法（语域判断 → 找钩子 → 设计画面 → 定版式 → 写 prompt）。**通用，fork 时不用动。** |
...
- **语域分治（信息 / 情绪 / 感悟）**：先判断内容语域——**信息/科技**走清晰、可上数字大字；**情绪/人文**走氛围、克制、借文章里的物做象征、**封面不上标语**；**感悟/散文**走纯意象、与文章细节解绑、无文字（Web UI 勾「感悟模式」或命令行 `--essay`）。手法服从语域，而非一刀切。
