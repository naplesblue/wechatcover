...
function transformListsForWechat(html) {
  ...
  while ((m = INNERMOST.exec(html))) {
    ...
    const liStyle = `margin: 0.5em 8px 0.5em ${8 + depth * 16}px; line-height: 1.75; color: #333;`;
    const bullet = depth === 0 ? '•' : '◦';
    const sections = [...m[2].matchAll(/<li[^>]*>([\s\S]*?)<\/li>/g)]
      .map((li, i) => {
        const prefix = m[1] === 'ol' ? `${i + 1}.` : bullet;
        return `<section style="${liStyle}">${prefix} ${li[1].trim()}</section>`;
      })
      .join('\n');
